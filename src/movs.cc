...
namespace peaq {
static auto constexpr ONE_POINT_FIVE_DB_POWER_FACTOR = 1.41253754462275;

struct noise_loudness_params
{
  double alpha;
  double thres_fac;
  double S0;
...
static auto calc_noise_loudness(
  noise_loudness_params const& params,
  ...
  std::array<double, EarModel::get_band_count()> const& test_excitation)
{
  ...
  auto band_count = EarModel::get_band_count();
  ...
  for (std::size_t i = 0; i < band_count; i++) {
    /* (67) in [BS1387] */
    auto sref = params.thres_fac * ref_modulation[i] + params.S0;
    auto stest = params.thres_fac * test_modulation[i] + params.S0;
    auto ethres = ear_model.get_internal_noise(i);
    auto ep_ref = ref_excitation[i];
    auto ep_test = test_excitation[i];
    /* (68) in [BS1387] */
    auto beta = exp(-params.alpha * (ep_test - ep_ref) / ep_ref);
    /* (66) in [BS1387] */
  ...
  if (noise_loudness < params.NLmin) {
    noise_loudness = 0.;
  }
...
template<typename EarModel>
static auto calc_modulation_difference(
  EarModel const& ear_model,
  typename ModulationProcessor<EarModel::get_band_count()>::state_t const&
    ref_mod_proc_state,
  typename ModulationProcessor<EarModel::get_band_count()>::state_t const&
  ...
  double levWt)
{
  ...
  return std::tuple{ mod_diff_1b, mod_diff_2b, temp_wt };
}
  ...
  movaccum_avg_window& mov_accum_win)
{
  ...
  for (std::size_t c = 0; c < mov_accum1.get_channels(); c++) {
    ...
    mov_accum_win.accumulate(c, mod_diff_1b);
  }
}
  ...
  movaccum_rms& mov_accum)
{
  ...
  for (std::size_t c = 0; c < mov_accum.get_channels(); c++) {
    ...
    mod_diff_2b *= 100. / band_count;
    mov_accum.accumulate(c, mod_diff_1b, temp_wt);
  }
}
  ...
  movaccum_rms& mov_accum)
{
  for (std::size_t c = 0; c < mov_accum.get_channels(); c++) {
    ...
    auto noise_loudness =
      calc_noise_loudness({ .alpha = 1.5, .thres_fac = 0.15, .S0 = 0.5, .NLmin = 0.0 },
                          ear_model,
  ...
  movaccum_rms_asym& mov_accum)
{
  for (std::size_t c = 0; c < mov_accum.get_channels(); c++) {
    ...
    auto const& test_excitation = level_state[c].get_adapted_test();
    auto noise_loudness =
      calc_noise_loudness({ .alpha = 2.5, .thres_fac = 0.3, .S0 = 1.0, .NLmin = 0.1 },
                          ear_model,
                          ref_mod_proc_state[c],
                          test_mod_proc_state[c],
                          ref_excitation,
  ...
  SWAP_MOD_PATTS_FOR_NOISE_LOUDNESS_MOVS
    auto missing_components =
      calc_noise_loudness({ .alpha = 1.5, .thres_fac = 0.15, .S0 = 1.0, .NLmin = 0.0 },
                          ear_model,
                          test_mod_proc_state[c],
                          ref_mod_proc_state[c],
                          test_excitation,
                     ...
                     typename EarModel::state_t const& test_state)
{
  ...
  auto noise_in_bands = ear_model.group_into_bands(noise_spectrum);
                     ...
                     movaccum_filtered_max& mov_accum_mfpd)
{
  ...
  for (std::size_t i = 0; i < band_count; i++) {
    ...
    auto detection_steps = -std::numeric_limits<double>::infinity();
  ...
  mov_accum_mfpd.accumulate(0, binaural_detection_probability);
}
} // namespace peaq
